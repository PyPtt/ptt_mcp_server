...
def _handle_ptt_exception(e: Exception, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    EXCEPTION_MAPPING = {
        ...
        PyPtt.NoSuchPost: ('在看板 {board} 中找不到文章 AID: {aid} 或 Index: {index}', 'NO_SUCH_POST'),
        ...
        PyPtt.NeedModeratorPermission: ('需要看板管理員權限', 'NEED_MODERATOR_PERMISSION'),
    }

    for exc_type, (message_format, code) in EXCEPTION_MAPPING.items():
        if isinstance(e, exc_type):
            message = message_format.format(**kwargs) if '{' in message_format else message_format
            return {'success': False, 'message': message, 'code': code}
    return {'success': False, 'message': f'操作時發生未知錯誤: {e}', 'args': kwargs, 'code': 'UNKNOWN_ERROR'}

def _call_ptt_service(session_storage_instance, method_name: str, success_message: Optional[str] = None, empty_data_message: Optional[str] = None, empty_data_code: Optional[str] = None, **kwargs) -> Dict[str, Any]:
